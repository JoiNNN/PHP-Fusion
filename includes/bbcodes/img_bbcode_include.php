   ...
   if (file_exists(LOCALE.LOCALESET."bbcodes/img.php")) {
      ...
      if (substr($matches[3], -1, 1) != "/") {
         ...
         if (function_exists('curl_init')) { // If cURL is available, use it.
            ...
            curl_setopt($curl, CURLOPT_TIMEOUT, 2); // Execution time allowed in seconds, to prevent page hanging.
            ...
            $content_size = (int)$content_size;
         	...
         	$content_type = get_headers($il,2);
         	$content_type = $content_type['Content-Type'];
            if (isset($content_type)) {
               $chksize = get_headers($il,1);
               $chksize = $chksize['Content-Length'];
               $chksize = (int)$chksize;
